...
async function fetchSeatData(semester, code) {
    ...
    code = encodeURI(code);
    return await fetch(`/api/v1/courses/code/${code}`).then((response) => response.json());
}

async function fetchCourseCodes() {
    return await fetch(`/api/v1/courses/courseCodes`).then((response) => response.json());
}

async function fetchSemesters() {
    return await fetch(`/api/v1/courses/semesters`).then((response) => response.json());
}
